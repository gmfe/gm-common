...
function usePagination<P extends Params = any, D extends Data = any>(
      ...
      if (!isUnmounted) {
        refState.current = _.merge(
          ...
          // 比如可能他们会有多语的字段，这样就污染了
          {
            ...
            count:
              pagingReq.offset === 0 ? pagingRes.count : refState.current.count,
          },
        )
      }
  ...
  const asyncResult = useAsync<P, D>(service, newOptions)

  const runChangePaging = (pagingReq: PagingReq) => {
    return asyncResult.run(
      Object.assign({}, asyncResult.params, {
        paging: {
